...
## Pipeline and CI setup

This repository now includes automated quality gates that match the quality plan:

- Pre-commit (local): `flake8` + `bandit`
- Pull Request CI: style, security, complexity, tests, coverage
- Release gate workflow: full re-check before demo/release

### 1) Local pre-commit gate (developer machine)

Added file: `.pre-commit-config.yaml`

What it blocks:

- Any `flake8` errors in backend/frontend source and tests
- Any high-severity findings from `bandit` in backend source
...
- `flake8 src tests`
- `bandit -r src -ll` (high severity gate)
- `python ../scripts/check_complexity.py --path src --max-complexity 8`
- `pytest --cov=src --cov-report=term-missing --cov-fail-under=70`

#### Backend non-blocking report

- `radon mi src -s` (maintainability index report)

#### Frontend checks

- `pytest tests`

### 3) Release gate workflow

Added file: `.github/workflows/release-gate.yml`
...
- Re-run all automated gates before demo/release in one workflow
- Remind about manual critical UX check:
  - report submission must work
  - machine status view must work

### 4) Complexity threshold enforcement (< 8)

Added file: `scripts/check_complexity.py`

Why this script exists:

- `radon` reports complexity but does not enforce the exact threshold directly in a stable way for this project requirement.
- This script parses `radon cc -j` output and fails CI if any function has complexity `> 8`.

### 5) Style configuration

Added file: `.flake8`

- Sets line length and excludes build/cache folders.
- Used both locally and in CI for consistent linting behavior.

### 6) Branch protection (GitHub settings)

To fully match the quality plan (“No approval from another team member”), enable branch protection for `main`:
...
Without branch protection, CI still runs, but approval enforcement is not guaranteed.